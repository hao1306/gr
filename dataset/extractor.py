...
def extract_image_position():
    f_scene = open('dataset/nuscenes/trian/v1.0-trainval/scene.json', 'r')
    f_sample = open('dataset/nuscenes/trian/v1.0-trainval/sample.json', 'r')
    f_sample_data = open('dataset/nuscenes/trian/v1.0-trainval/sample_data.json', 'r')
    f_ego_pose = open('dataset/nuscenes/trian/v1.0-trainval/ego_pose.json', 'r')
    ...
    for i, sc in enumerate(scene):
        f_scene = open('dataset/nuscenes/image_scene/'+sc['name']+'.csv', 'w')
        for s in sample:
...
if __name__ == '__main__':
    ...
    filepaths = glob.glob('dataset/nuscenes/image_scene/*.csv')
    for filepath in filepaths:
        filename = os.path.basename(filepath)
        print(filename)
        if filename[:-4] in mini_train:
            shutil.copy(filepath, 'dataset/nuscenes/image_scene/mini_train/'+filename)
        if filename[:-4] in mini_val:
            shutil.copy(filepath, 'dataset/nuscenes/image_scene/mini_val/'+filename)

        if filename[:-4] in val:
            shutil.move(filepath, 'dataset/nuscenes/image_scene/val/'+filename)
        else:
            shutil.move(filepath, 'dataset/nuscenes/image_scene/train/'+filename)
